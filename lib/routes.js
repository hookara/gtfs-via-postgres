...
const beforeAll = (opt) => {
	...
	return `\
        DO $$
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'route_type_val') THEN
            ...
            END IF;
            IF NOT EXISTS (SELECT 1 FROM pg_cast WHERE castsource = '${opt.schema}.route_type_val'::regtype AND casttarget = 'text'::regtype) THEN
                CREATE CAST ("${opt.schema}".route_type_val AS text) WITH INOUT AS IMPLICIT;
            END IF;

            CREATE TABLE IF NOT EXISTS "${opt.schema}".routes (
            ...
            DROP TABLE IF EXISTS "${opt.schema}".temp_routes;
            CREATE TABLE "${opt.schema}".temp_routes (LIKE "${opt.schema}".routes INCLUDING ALL);
        END $$;

        ${opt.postgraphile ? `COMMENT ON TYPE "${opt.schema}".route_type_val IS E'@enum\\n@enumName RouteType\\n';` : ''}

        COPY "${opt.schema}".temp_routes (
            route_id,
            agency_id,
            route_short_name,
            route_long_name,
...
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_route_short_name ON "${opt.schema}".routes (route_short_name);

    ${opt.postgraphile ? `CREATE INDEX IF NOT EXISTS idx_agency_id ON "${opt.schema}".routes (agency_id);` : ''}

    -- Insérer les données depuis la table temporaire
    INSERT INTO "${opt.schema}".routes
    SELECT * FROM "${opt.schema}".temp_routes
