...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_type WHERE typname = 'wheelchair_accessibility'
    ) THEN
    ...
    IF NOT EXISTS(
        SELECT 1 FROM pg_type WHERE typname = 'bikes_allowance'
    ) THEN
    ...
    CREATE TABLE IF NOT EXISTS "${opt.schema}".trips (
    ...
    DROP TABLE IF EXISTS "${opt.schema}".temp_trips;
    CREATE TABLE "${opt.schema}".temp_trips (LIKE "${opt.schema}".trips INCLUDING ALL);
END $$;

COPY "${opt.schema}".temp_trips (
    trip_id,
    route_id,
    service_id,
    trip_headsign,
...
const formatTripsRow = (t) => {
	return [
		...
		t.block_id || null,
		t.shape_id || null,
		t.wheelchair_accessible ? wheelchairAccessibility(t.wheelchair_accessible) : null,
...
DO $$
BEGIN
    ${opt.postgraphile ? `CREATE INDEX IF NOT EXISTS idx_route_id ON "${opt.schema}".trips (route_id);`: ''}
    ${opt.postgraphile ? `COMMENT ON TABLE "${opt.schema}".trips IS E'@foreignKey (shape_id) references shapes_aggregated|@fieldName shape';` : ''}

    -- Insérer les données depuis la table temporaire
    INSERT INTO "${opt.schema}".trips
    SELECT * FROM "${opt.schema}".temp_trips
